...
class Preview extends RelativeLayout implements SurfaceHolder.Callback {
    ...
    public void switchCamera(Camera camera, int cameraId) {
        try {
            ...
            mPreviewSize = getOptimalPreviewSize(camera.getParameters().getSupportedPreviewSizes(), width, height);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (changed && getChildCount() > 0) {
            //TODO: change to search instanceOf CustomSurfaceView
            ...
            if(getDisplayOrientation() == 90 || getDisplayOrientation() == 270) {
                ...
                if(getDisplayOrientation() == 0) {
                    this.layout(0,0, height, width);
                    child.layout(0,0, height, width);
                } else if(getDisplayOrientation() == 180) {
                    this.layout(this.getTop(), this.getLeft(), height, width);
                    child.layout(this.getTop(), this.getLeft(), height, width);
                }

            }

            Log.d("layout", "right:" + width);
            Log.d("layout", "bottom:" + height);
        }

        Log.d("THIS", "getWidth:" +  this.getWidth());
        Log.d("THIS", "getHeight:" +  this.getHeight());
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        if(mCamera != null) {
            try {
                ...
                // the preview.
                if (mCamera.getParameters().getSupportedPreviewSizes() != null) {
                    mPreviewSize = getOptimalPreviewSize(mCamera.getParameters().getSupportedPreviewSizes(), w, h);
                }
